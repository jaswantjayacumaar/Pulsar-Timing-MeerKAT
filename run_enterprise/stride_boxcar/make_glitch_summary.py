...
def run(psr, rootdir = "/nvme1/yliu/yangliu/yang/"):

    dirs={"std":"models", "glf2":"glf2"}
    prefixes={"std":"bst", "glf2":"glf"}
    ...
    with open("{}_sum.tex".format(psr),"w") as outfile:
        ...
        table_started=False
        for glitch in range(10):
            glitch_started=False
            for ty in ["std","glf2"]:
                proot = os.path.join(rootdir,dirs[ty],psr)
                for recoveries in [0,1,2,3]:
                    if glitch==0:
                        ...
                        glid="{}".format(glitch)
                        glitch_val = glitch
                    suffix="{}{}".format(glid,suffixes[recoveries])
                    res_file = os.path.join(proot,"{}_{}_{}.par.results".format(prefixes[ty],psr,suffix))
                    ...
                    if os.path.exists(corner_pdf):
                        fig_ref="{}_{}_{}".format(ty,psr,suffix)
        ...
        comments=os.path.join(rootdir,"{}_comments.txt".format(psr))            
        commentsm=os.path.join(rootdir,"models",psr,"{}_comments.txt".format(psr))
        if os.path.exists(comments):
            with open(comments) as f:
                outfile.writelines(f.readlines())
        elif os.path.exists(commentsm):
            with open(commentsm) as f:
                outfile.writelines(f.readlines())
        else:
            print("\n\n >> No comments file {}".format(comments))

        nunudot=os.path.join(rootdir,"models",psr,"nu_nudot_gp_{}.pdf".format(psr))
        if os.path.exists(nunudot):
            ...
            nunudot=os.path.join(rootdir,"glf2",psr,"nu_nudot_gp_{}.pdf".format(psr))
            if os.path.exists(nunudot):
                outfile.write(r"\begin{figure}[b]")
                outfile.write("\n")
                outfile.write("\includegraphics[width=\\textwidth,height=0.95\\textheight,keepaspectratio]{{{}}}\n".format(nunudot))
...
def read_results_file(res_file):
    ...
    with open(res_file) as f:
        f.readline()
        for line in f:
            e=line.strip().rsplit(maxsplit=10)
